...
class BuildNet(BaseModule):
    ...
    def forward(self, x,return_loss=True,return_logits=False,**kwargs):
        if return_logits:
            x = self.extract_feat(x)
            logits = self.head.simple_test(x, softmax=False, post_process=False)
            return logits
        else:
            if return_loss:
    ...
    def forward_logits(self, x, **kwargs):
        out = self.extract_feat(x)
        return out
