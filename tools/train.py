...
def main():
    ...
    # 训练
    post_process=None
    if 'mixup' in args.config and 'cutmix' in args.config:
        print("mixup and cutmix")
        cutmix = v2.CutMix(alpha=0.2,num_classes=7)
        mixup = v2.MixUp(alpha=0.2,num_classes=7)
        cutmix_or_mixup = v2.RandomChoice([cutmix, mixup])
        post_process = cutmix_or_mixup
    elif 'mixup' in args.config:
        print("mixup")
        mixup = v2.MixUp(alpha=0.2,num_classes=7)
        post_process = mixup
    elif 'cutmix' in args.config:
        print("cutmix")
        cutmix = v2.CutMix(alpha=0.2,num_classes=7)
        post_process = cutmix
