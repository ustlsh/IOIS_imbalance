...
def main(): 
    ...
    test_annotations    = 'datas/train.txt'
    ...
    with torch.no_grad():
        ...
        with tqdm(total=len(test_datas)//data_cfg.get('batch_size')) as pbar:
            for _, batch in enumerate(test_loader):
                ...
                if torch.argmax(outputs).item()==target.item():
                    correct = correct + 1
    ...
    y_preds = torch.argmax(torch.cat(preds),dim=1)
    print(y_preds.size(), torch.cat(targets).size())
    bacc = balanced_accuracy_score(torch.cat(targets).cpu(), y_preds.cpu())
    print("bacc", bacc)
...
# CUDA_VISIBLE_DEVICES=0 python tools/evaluation.py /home/slidm/OCTA/Awesome-Backbones/models/resnet/resnet50_isic_flip_rot_rc_eval.py