...
def train(model, runner, lr_update_func, device, epoch, epoches, meta):
    ...
    with tqdm(total=len(runner.get('train_loader')),desc=f'Train: Epoch {epoch + 1}/{epoches}',postfix=dict,mininterval=0.3) as pbar:
        for iter, batch in enumerate(runner.get('train_loader')):            
            ...
            prob = random.random()
            '''
            if post_process and prob > 0.8:
                images, targets = post_process(images, targets)
            with torch.no_grad():
...
def validation(model, runner, cfg, device, epoch, epoches, meta):
    ...
    s_dir = os.path.join(meta['save_dir'],'preds')
    
    if not os.path.exists(s_dir):
        os.makedirs(s_dir)
    s_path = os.path.join(s_dir,'Epoch{:03}.npz'.format(epoch+1))
    preds_arr = np.array(torch.cat(preds).cpu())
    targets_arr = np.array(torch.cat(targets).cpu())
    print(preds_arr.shape, targets_arr.shape)
    np.savez(s_path, y_pred=preds_arr, y_true=targets_arr)
    
    eval_results = evaluate(torch.cat(preds),torch.cat(targets),cfg.get('metrics'),cfg.get('metric_options'))

    # specific for isic 2018
    acc = np.array([0. for c in range(7)])
    arr_t = np.array(torch.cat(targets).cpu())
    for c in range(7):
        # torch.cat(preds): [1504,7], torch.cat(targets): [1504]
        #print(torch.cat(preds)[0], arr_t[0])
        #print(max((arr_t == c).sum(), 1))
        acc[c] = ((torch.argmax(torch.cat(preds),dim=1) == torch.cat(targets)) * (torch.cat(targets) == c)).float().sum() / max((torch.cat(targets) == c).sum(), 1)

    # save each class acc to meta
    
    meta['val_perclass_acc']['class0'].append(acc[0])
    meta['val_perclass_acc']['class1'].append(acc[1])
    meta['val_perclass_acc']['class2'].append(acc[2])
    meta['val_perclass_acc']['class3'].append(acc[3])
    meta['val_perclass_acc']['class4'].append(acc[4])
    meta['val_perclass_acc']['class5'].append(acc[5])
    meta['val_perclass_acc']['class6'].append(acc[6])
    

    meta['train_info']['val_acc'].append(eval_results)

    with torch.no_grad():
        with tqdm(total=len(runner.get('val_train_loader')), desc=f'Test : Epoch {epoch + 1}/{epoches}',mininterval=0.3) as pbar:
            for iter, batch in enumerate(runner.get('val_train_loader')):
                images, target, _ = batch
                outputs = model(images.to(device),return_loss=False)
                preds.append(outputs)
                targets.append(target.to(device))   
    ...
    eval_train_results = evaluate(torch.cat(preds),torch.cat(targets),cfg.get('metrics'),cfg.get('metric_options'))
    ...
    meta['train_perclass_acc']['class0'].append(acc[0])
    meta['train_perclass_acc']['class1'].append(acc[1])
    meta['train_perclass_acc']['class2'].append(acc[2])
    meta['train_perclass_acc']['class3'].append(acc[3])
    meta['train_perclass_acc']['class4'].append(acc[4])
    meta['train_perclass_acc']['class5'].append(acc[5])
    meta['train_perclass_acc']['class6'].append(acc[6])

    meta['train_info']['train_acc'].append(eval_train_results)
